function [pi_d,pi_u,qd_sol] = NiN_profits(zd,zu,qum,q_um,q_m,q_d,par)
...
% par: various paramters
    qum = exp(qum); % for a positive number solution.
    qd_old = 1;
    gap = 1;
    while gap>1e-6
...
%     qd_sol = fsolve(@(qd) eq_qd(zd,qum,q_um,q_m,q_d,qd,par),qd_old);
    ...
    cd = unit_cost(pm,p_m,par);    
    [epsilond,~] = cournot_markup(cd,qd_sol,par);
    pi_d = (1/(epsilond-1))*(cd/zd)*qd_sol;
    w = par.w;
