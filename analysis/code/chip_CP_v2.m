...
% index: (1,...S-4 = chemical sector, S-3 = electronic device sector, L_chip, H_chip, PR).
    ...
    z(:,1:S-3)  = upsilon(:,1:S-3).*(repmat(w,1,S-3).^(1-gamma_sum(:,1:S-3))).*P_prod(:,1:S-3); % get c for usual sectors
    z(:,S-2)    = upsilon(:,S-2).*(w.^(1-gamma_sum(:,S-2)-gamma(:,S-1,S-2))).*P_prod(:,S-2) ...
        .*P(:,S-1).^(gamma(:,S-1,S-2));  % contribution of unit price of high-tier chips
    z(:,S-1)    = upsilon(:,S-1).*(w.^(1-gamma_sum(:,S-1))).*P_prod_nochem(:,S-1) ... 
...
function z = Xeq(w,P,Y_prev,par,S,J,L,D)
    ...
    Y_gamma = sum(Y_temp(:,:,1:end-2).*gamma(:,:,1:end-2),3);
    Y_gamma = reshape(Y_gamma(:),J,S);
    z       = Y_gamma+alpha.*repmat(w.*L+D,1,S);
    z(:,S-3)= z(:,S-3)+gamma(:,S-2,S-3) ...
        .*(par.K*P(:,end))./(par.K*P(:,end)+P(:,S-4)).*Y_prev(:,S-2);
    z(:,S-1)=gamma(:,S-2,S-1).*Y_prev(:,S-2);
    z(:,S)=gamma(:,S-3,S-1).*(par.K*P(:,end))./(par.K*P(:,end)+P(:,S-3)).*Y_prev(:,S-1);
    z = z(:);
end
