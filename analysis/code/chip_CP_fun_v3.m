...
solution.pi = pieq(cP_prev,tau,par_fixed); %pieq(cP,tau,par_fixed)
...
%     P_prod_nochem   = reshape(prod(P_mygamma(:,(1:S-4),:),2).*P_mygamma(:,S-2,:),J,S);  
    ...
    z(:,1:S-3)  = (repmat(w,1,S-3).^(1-mygamma_sum(:,1:S-3))).*P_prod(:,1:S-3); % get c for usual sectors
    z(:,S-2)    = (w.^(1-mygamma_sum(:,S-2)-mygamma(:,S-1,S-2))) ...
        .*(P(:,S-1).*delta+P(:,S-1).*(1-delta)).^(mygamma(:,S-1,S-2));  % delta: cost share of HF and PR composition.
    z(:,S-1)    = w; % contribution of unit price of PR-Chemical bundle
    ...
    z(J*S+1:end) = reshape((THETA_sum).^(-1./theta_rep),J,S); % get P
    z       = reshape(z,2*J*S,1);
end
...
function z = pieq(cP,tau,par_fixed)
    ...
    P = exp(mididx(cP(S*J+1:end),S,J,J));
