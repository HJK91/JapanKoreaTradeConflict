...
N = 2*1e+2; % # of goods in each country
...
while k <= K
    ...
    while n <=N
        ...
        [C_matrix(k,n,j),~,flag(k,n,j)] = fsolve(@(C) G(C,q,myPhi,theta,K), C0);
        if flag(k,n,j)==1
            n=n+1;
        else
            C0          = exp(randn(1,1));
...
save('firmdraws_AB.mat','C_matrix')
%%
X = 1;
Y0 = randn(K*N*J,1);
...
for j = 1:J
    ...
    for jj = 1:J
        C_mat_D1((jj-1)*K+1:jj*K,:) = C_matrix(:,:,jj)*D(jj,j);
        if jj == 2 && j == 3
            C_mat_D2((jj-1)*K+1:jj*K,:) = C_matrix(:,:,jj)*D(jj,j)*1.56;
        else
            C_mat_D2((jj-1)*K+1:jj*K,:) = C_mat_D1((jj-1)*K+1:jj*K,:);
        end
    end
    Y_sol1(:,j) = fsolve(@(Y) FOC(C_mat_D1,Y,X,rho,eta,K,N,J),Y0);
    Y_sol2(:,j) = fsolve(@(Y) FOC(C_mat_D2,Y,X,rho,eta,K,N,J),Y0);
    Y_sol1(:,j) = exp(Y_sol1(:,j));
    Y_sol2(:,j) = exp(Y_sol2(:,j));
    P_sol1(:,j) = P_agg(C_mat_D1,reshape(Y_sol(:,j),K*J,N),rho,eta);
    P_sol2(:,j) = P_agg(C_mat_D2,reshape(Y_sol(:,j),K*J,N),rho,eta);
end
%
Phat3 = P_sol2./P_sol1;
